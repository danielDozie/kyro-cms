#!/usr/bin/env node
import Te from'bcryptjs';import Je,{randomBytes,randomUUID}from'crypto';import X,{readFileSync,existsSync,mkdirSync,writeFileSync}from'fs';import z,{dirname,join,resolve}from'path';import {createRequire}from'module';import {pgTable,timestamp,jsonb,integer,boolean,varchar,uuid,uniqueIndex,index,text}from'drizzle-orm/pg-core';import {sql,eq,desc,and,gt as gt$1}from'drizzle-orm';import {Command}from'commander';import {fileURLToPath}from'url';import {glob}from'glob';import {execSync}from'child_process';import Nt from'prompts';import u from'chalk';import K from'ora';var At=Object.defineProperty;var x=(i,e)=>()=>(i&&(e=i(i=0)),e);var re=(i,e)=>{for(var t in e)At(i,t,{get:e[t],enumerable:true});};var Ze={};re(Ze,{SQLiteAuthAdapter:()=>q});function Dt(){return ne||(ne=createRequire("file:///")("node:sqlite").DatabaseSync,ne)}var ne,vt,Mt,$t,Ft,q,Ee=x(()=>{vt=5e3,Mt=1e3,$t=-64e3,Ft=268435456,q=class{db=null;path;saltRounds;externalDb;busyTimeout;walAutoCheckpoint;cacheSize;mmapSize;preparedStatements=new Map;constructor(e={}){this.path=e.path||"./data/auth.db",this.saltRounds=e.saltRounds||12,this.externalDb=!!e.db,this.busyTimeout=e.busyTimeout??vt,this.walAutoCheckpoint=e.walAutoCheckpoint??Mt,this.cacheSize=e.cacheSize??$t,this.mmapSize=e.mmapSize??Ft,e.db&&(this.db=e.db);}async connect(){if(this.db)return;let e=dirname(this.path);e&&e!=="."&&mkdirSync(e,{recursive:true}),this.db=new(Dt())(this.path),this.db.exec(`PRAGMA busy_timeout = ${this.busyTimeout}`),this.db.exec("PRAGMA journal_mode = WAL"),this.db.exec("PRAGMA synchronous = NORMAL"),this.db.exec("PRAGMA cache_size = "+this.cacheSize),this.db.exec("PRAGMA mmap_size = "+this.mmapSize),this.db.exec("PRAGMA wal_autocheckpoint = "+this.walAutoCheckpoint),this.db.exec("PRAGMA foreign_keys = ON"),this.db.exec("PRAGMA temp_store = MEMORY"),this.ensureTables(),this.prepareStatements();}async disconnect(){this.db&&!this.externalDb&&(this.db.exec("PRAGMA wal_checkpoint(TRUNCATE)"),this.db.close(),this.db=null,this.preparedStatements.clear());}async ensureConnected(){if(this.db||await this.connect(),!this.db)throw new Error("Failed to connect to SQLite database");return this.db}ensureTables(){if(this.db){this.db.exec(`
...
`,e}function Ge(){return new Command("generate").description("Generate TypeScript types from collection configs").option("-c, --config <path>","Path to config file or directory","./src/kyro").option("-o, --output <path>","Output file path","./src/types/kyro.d.ts").option("-w, --watch","Watch for changes and regenerate").option("--url <url>","URL of a running Kyro CMS instance (enables remote mode)").option("--api-key <key>","API key for authentication (required with --url)").action(async e=>{if(e.url){e.url||(console.error("\u274C --url is required when using --api"),process.exit(1)),e.apiKey||(console.error("\u274C --api-key is required when using --api"),process.exit(1));let r=resolve(e.output);try{let n=await kt(e.url,e.apiKey),o=It(n),a=join(r,"..");existsSync(a)||mkdirSync(a,{recursive:!0}),writeFileSync(r,o);}catch(n){console.error("\u274C Error:",n.message),process.exit(1);}return}let t=resolve(e.config),s=resolve(e.output);try{let r=await glob(`${t}/**/*.{ts,js}`);r.length===0&&(console.error("\u274C No config files found at:",t),process.exit(1));let n=[];for(let c of r)try{let d=await import(c),f=Object.values(d).filter(l=>l&&l.slug&&l.fields);n.push(...f);}catch{console.warn(`\u26A0\uFE0F  Could not parse: ${c}`);}n.length===0&&(console.error("\u274C No valid collection configs found"),process.exit(1));let o=Ve(n),a=join(s,"..");if(existsSync(a)||mkdirSync(a,{recursive:!0}),writeFileSync(s,o),e.watch){let{watch:c}=await import('fs');c(t,{recursive:!0},async(d,f)=>{let l=[];for(let O of r)try{let A=await import(O+`?t=${Date.now()}`),S=Object.values(A).filter(I=>I&&I.slug&&I.fields);l.push(...S);}catch{}let b=Ve(l);writeFileSync(s,b);});}}catch(r){console.error("\u274C Error:",r.message),process.exit(1);}})}function Lt(i=3){return Je.randomBytes(i).toString("hex")}function Ct(){return Je.randomBytes(12).toString("base64").replace(/[^a-zA-Z0-9]/g,"").slice(0,16)}function Qe(){let i=new Command("deploy").description("Deploy Kyro CMS");return i.command("cloudflare").description("Deploy to Cloudflare Workers with Assets").option("-d, --database <type>","Database type (d1|postgres)").option("-u, --database-url <url>","PostgreSQL connection string").option("-n, --name <name>","Cloudflare project name").option("-b, --r2-bucket <name>","R2 storage bucket name").option("-e, --email <email>","Initial Super Admin email").option("-p, --password <password>","Initial Super Admin password").option("-y, --non-interactive","Skip all prompts and use defaults").option("-q, --quiet","Run without prompts or spinners").option("-j, --json","Emit a final JSON line with deploy results (for programmatic consumers)").action(async e=>{console.log(`
  ${u.cyan.bold("\u2726 Kyro CMS")} ${u.dim("\u2014 Cloudflare Deployment")}
`);let t=Lt(),s=Ct(),{database:r,databaseUrl:n,name:o,r2Bucket:a,email:c,password:d,nonInteractive:f,quiet:l,json:b}=e;l&&(f=true);let O=`kyro-postgres-hd-${t}`;if(n&&!r&&(r="postgres"),!f&&process.stdout.isTTY){let h=[];r||h.push({type:"select",name:"database",message:"Select database infrastructure:",choices:[{title:"Cloudflare D1 (Native Serverless SQLite, auto-provisioned)",value:"d1"},{title:"PostgreSQL (External DB via Cloudflare Hyperdrive)",value:"postgres"}],initial:0}),r==="postgres"&&!n&&h.push({type:"text",name:"databaseUrl",message:"PostgreSQL Connection URL:"}),o||h.push({type:"text",name:"name",message:"Cloudflare Project Name:",initial:`kyro-app-${t}`}),a||h.push({type:"text",name:"r2Bucket",message:"Cloudflare R2 Bucket Name:",initial:`kyro-media-${t}`}),c||h.push({type:"text",name:"email",message:"Initial Super Admin Email:",initial:"admin@kyro-cms.com"}),d||h.push({type:"text",name:"password",message:"Initial Super Admin Password (leave blank to auto-generate):"});let m=await Nt(h);m.database&&(r=m.database),m.databaseUrl&&(n=m.databaseUrl),m.name&&(o=m.name),m.r2Bucket&&(a=m.r2Bucket),m.email&&(c=m.email),m.password&&(d=m.password);}o=o||`kyro-app-${t}`,a=a||`kyro-media-${t}`,c=c||"admin@kyro-cms.com",d=d||s,r=r||"d1",console.log(u.bgGray.black.bold(`
 Deployment Plan `)),console.log(`  ${u.dim("\u251C\u2500")} Hosting   : ${u.cyan("Cloudflare Workers with Assets")}`),console.log(`  ${u.dim("\u251C\u2500")} Database  : ${u.cyan(r==="d1"?"Cloudflare D1 (Native)":"PostgreSQL (Hyperdrive)")}`),console.log(`  ${u.dim("\u251C\u2500")} Project   : ${u.cyan(o)}`),console.log(`  ${u.dim("\u251C\u2500")} R2 Bucket : ${u.cyan(a)}`),console.log(`  ${u.dim("\u2514\u2500")} Admin     : ${u.cyan(c)}
`);let S=X.existsSync(z.join(process.cwd(),"pnpm-lock.yaml"))?"pnpm":"npm",I="npx wrangler";try{execSync(`${I} whoami`,{stdio:"ignore"}),console.log(`  ${u.green("\u2714")} Cloudflare Wrangler authenticated`);}catch{console.log(`  ${u.red("\u2716")} Cloudflare Wrangler authentication required.`),console.log(`    Run ${u.bold(`${I} login`)} or set the ${u.bold("CLOUDFLARE_API_TOKEN")} env var.`),process.exit(1);}let Z="",ee="",$=l?{isSilent:true}:{};if(r==="postgres"){n||(console.log(`  ${u.red("\u2716")} PostgreSQL mode requires a database URL.`),process.exit(1));let h=K({text:"Checking existing Hyperdrive resources...",...$}).start();try{let m=execSync(`${I} hyperdrive list --json`,{stdio:"pipe"}).toString(),C=JSON.parse(m.slice(m.indexOf("["))).find(We=>We.name===O);C&&(ee=C.id||C.uuid),h.succeed("Hyperdrive check complete");}catch{h.fail("Failed to list Hyperdrive resources");}if(!ee){let m=K({text:"Creating Hyperdrive...",...$}).start();try{let H=execSync(`${I} hyperdrive create "${O}" --connection-string="${n}" --json`,{stdio:"pipe"}).toString(),C=JSON.parse(H.slice(H.indexOf("{")));ee=C.id||C.uuid,m.succeed("Hyperdrive created");}catch{m.fail("Failed to create Hyperdrive");}}console.log(`  ${u.green("\u2714")} Hyperdrive resource ready (${O})`);}else {let h=`${o}-d1`,m=K({text:"Creating D1 database...",...$}).start();try{let C=execSync(`${I} d1 create "${h}"`,{stdio:"pipe"}).toString().match(/([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/i);C&&(Z=C[1]),m.succeed("D1 database created");}catch{try{let C=execSync(`${I} d1 list --json`,{stdio:"pipe"}).toString(),ge=JSON.parse(C).find(bt=>bt.name===h);ge&&(Z=ge.id||ge.uuid),m.succeed("Existing D1 database found");}catch{m.fail("Failed to create/find D1 database");}}Z||(console.log(`  ${u.red("\u2716")} Failed to create/find D1 database '${h}'. (D1 database limit may be reached).`),process.exit(1));}let Be=K({text:"Creating R2 bucket...",...$}).start();try{execSync(`${I} r2 bucket create "${a}"`,{stdio:"ignore"}),Be.succeed("R2 bucket created");}catch{Be.fail("Failed to create R2 bucket");}try{execSync(`echo "y" | ${I} r2 bucket dev-url enable "${a}"`,{stdio:"ignore"});}catch{}let Et=K({text:"Generating wrangler.toml...",...$}).start(),te=`name = "${o}"
...
bucket_name = "${a}"
`,X.writeFileSync(z.join(process.cwd(),"wrangler.toml"),te,"utf8"),Et.succeed("wrangler.toml generated");let _t=`import bcrypt from 'bcryptjs'; console.log(bcrypt.hashSync('${d}', 10));`,He="";try{He=execSync(`node -e "${_t}"`,{stdio:"pipe"}).toString().trim();}catch{}if(r==="postgres"){try{execSync(`DATABASE_URL="${n}" npx drizzle-kit push --force`,{stdio:"ignore"});}catch{}let h=`
          ...
          bootstrap();
        `;try{execSync(`node -e "${h.replace(/\n/g," ")}"`,{stdio:"pipe"});}catch{}console.log(`  ${u.green("\u2714")} PostgreSQL schema migrated & Super Admin seeded`);}else {let h=`
          CREATE TABLE IF NOT EXISTS users (
            id TEXT PRIMARY KEY,
            email TEXT UNIQUE NOT NULL,
            password_hash TEXT,
        ...
        `;try{execSync(`${I} d1 execute "${o}-d1" --remote --command="${h.replace(/\n/g," ")}"`,{stdio:"ignore"}),console.log(`  ${u.green("\u2714")} D1 schema migrated & Super Admin seeded`);}catch{}}let Xe=K({text:"Building Astro project...",...$}).start();try{execSync(`${S} run build`,{stdio:"inherit"});let h=z.join(process.cwd(),"wrangler.toml"),m=X.readFileSync(h,"utf8");m.includes('main = "dist/server/entry.mjs"')||X.writeFileSync(h,`main = "dist/server/entry.mjs"
`+m,"utf8"),Xe.succeed("Build complete");}catch{Xe.fail("Build failed"),console.log(`
  ${u.red("\u2716")} Build failed. Inspect output above.`),process.exit(1);}let Ye=K({text:"Deploying to Cloudflare Workers...",...$}).start();X.existsSync(z.join(process.cwd(),".wrangler"))&&X.rmSync(z.join(process.cwd(),".wrangler"),{recursive:true,force:true});let se="";try{let h=execSync(`${I} deploy`,{stdio:"pipe"}).toString();process.stdout.write(h);let m=h.match(/https:\/\/[a-zA-Z0-9._-]+\.workers\.dev/);m&&(se=m[0]),Ye.succeed("Deployment successful"),console.log(`
  ${u.green.bold("\u{1F389} Deployment Successful!")}
`),console.log(`  ${u.bold("Super Admin Credentials")}`),console.log(`  ${u.dim("\u251C\u2500")} ${u.bold("Email   :")} ${u.cyan(c)}`),console.log(`  ${u.dim("\u2514\u2500")} ${u.bold("Password:")} ${u.yellow.bold(d)}`),se&&console.log(`  ${u.dim("\u2514\u2500")} ${u.bold("Live URL:")} ${u.cyan(se)}`),console.log(`
  ${u.dim("\u26A0\uFE0F  Save these credentials. Password won't be shown again.")}
