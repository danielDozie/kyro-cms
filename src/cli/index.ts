...
program
  ...
  .action(async (options) => {
    ...
    // Locate SSR server entrypoint generated by @astrojs/node (index.mjs or entry.mjs)
    const adminDir = join(process.cwd(), "admin");
    const candidates = [
      join(adminDir, "dist", "server", "index.mjs"),
      join(adminDir, "dist", "server", "entry.mjs"),
      join(process.cwd(), "dist", "server", "index.mjs"),
      join(process.cwd(), "dist", "server", "entry.mjs"),
    ];

    const entryNode = candidates.find((p) => existsSync(p));

    console.log(`🚀 Starting Kyro CMS on ${options.host}:${options.port}`);

    let child;
    if (entryNode) {
      console.log(`✓ Serving via standalone entrypoint: ${entryNode}`);
      child = spawn("node", [entryNode], {
        env: { ...process.env, HOST: options.host, PORT: options.port, NODE_ENV: "production" },
        stdio: "inherit",
      });
    } else {
      console.warn(`⚠️ Standalone entrypoint not found, falling back to astro preview in ${adminDir}`);
      child = spawn(
        "node",
        ["./node_modules/.bin/astro", "preview", "--port", options.port, "--host", options.host],
        {
          cwd: existsSync(adminDir) ? adminDir : process.cwd(),
