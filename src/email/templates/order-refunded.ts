// ============================================================================
// E-commerce Order Refunded Template
// ============================================================================

import type { BrandConfig } from "../base.js";
import { renderBaseLayout } from "../base.js";

export function renderOrderRefunded(
  orderId: string,
  customerName = "Customer",
  refundAmount: string,
  brandConfig?: BrandConfig,
) {
  const subject = `Refund Processed for Order #${orderId}`;
  const bodyHtml = `
    <p class="email-text" style="margin: 0 0 16px; font-size: 14px; line-height: 1.6; color: #52525b;">
      Hi <strong class="email-strong" style="color: #09090b;">${customerName}</strong>,
    </p>
    <p class="email-text" style="margin: 0 0 20px; font-size: 14px; line-height: 1.6; color: #52525b;">
      We have processed a refund for your order <strong>#${orderId}</strong>. It may take 3-5 business days for the funds to appear on your original payment method.
    </p>

    <table role="presentation" width="100%" border="0" cellspacing="0" cellpadding="0" class="email-table" style="background-color: #fafafa; border: 1px solid #e4e4e7; border-radius: 8px; margin-bottom: 24px; padding: 16px; border-collapse: separate;">
      <tr>
        <td style="font-size: 13px; color: #09090b; line-height: 1.6;" class="email-value">
          💸 <strong>Refund Amount:</strong> ${refundAmount}
        </td>
      </tr>
    </table>
  `;

  const html = renderBaseLayout({
    brand: brandConfig,
    title: "Refund Processed",
    previewText: `We have processed a refund of ${refundAmount} for order #${orderId}.`,
    bodyHtml,
    ctaText: "View Store",
    ctaUrl: "https://kyro-cms.com",
  });

  const text = `Hi ${customerName},\n\nWe have processed a refund of ${refundAmount} for order #${orderId}.\n\nView store: https://kyro-cms.com`;

  return { subject, html, text };
}
